...
function remove_c_tools() {
  echo "Removendo ferramnetas de desenvolvimento C"
  dependent_packages="build-essential gdb libc6-dev libc-dev gcc g++ make dpkg-dev"

  for package in $dependent_packages; do
    if apt list --installed | grep -q "$package"; then
      echo "Removendo $package"
      sudo apt purge $package
      echo "$package Removido com sucesso! Pressione qualquer tecla para continuar: "
      read -n 1 -s
    else
      echo "$package não está instaldo ou não foi encontrado! Pressione qualquer tecla para continuar: "
      read -n 1 -s
    fi
  done
...
function install_build_essentials() {
  echo "Instalando ferramentas de desenvolvimento ecenssiais... "
  sleep 1
  apt_update

  if sudo apt install build-essential gdb -y; then
    echo "build-essential e gdb instalados com sucesso! Pressione qualquer tecla para continuar:"
    read -n 1 -s
  else
    echo "Falha ao instalar build-essential e gdb. Pressione qualquer tecla para continuar: "
    read -n 1 -s
    return 1
  fi
}
